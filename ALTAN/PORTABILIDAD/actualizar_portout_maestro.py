# PORTABILIDAD/actualizar_portout_maestro.py
import time
import warnings
...
def ejecutar_pipeline_portout():
    ...
    sql_drop_temp = "DROP TABLE IF EXISTS analysis_aftersale.amv_portout_temp PURGE"
    sql_drop_main = "DROP TABLE IF EXISTS analysis_aftersale.amv_portout PURGE"
    
    # Paso 2: Respaldo de histórico 
    sql_respaldo = """
    CREATE TABLE IF NOT EXISTS analysis_aftersale.amv_portout_temp
    AS
    SELECT 
    *
    ...
    sql_principal = """
    CREATE TABLE IF NOT EXISTS analysis_aftersale.amv_portout
    AS
    WITH port AS (
        SELECT 
             be_id
        ...
        AND operation = 'Portabilidad OUT C'	
    )
    , cat_dida AS ( 
        SELECT 
    ...
    , ini AS (
        SELECT 
            ...
            , UPPER(TRIM(COALESCE(cr_fallback.nombre_rida, cat_dida.nombre_dida ))) AS nombre_receptor
    ...
    SELECT 
        ...
        , IF (ini.operation IREGEXP 'OUT', NULL, DATEDIFF(ini.fecha_porta, ini.active_date)) AS t_act_port
    ...
    # Paso 5: Reintegración final
    sql_insert_final = "INSERT INTO analysis_aftersale.amv_portout SELECT * FROM analysis_aftersale.amv_portout_temp"

    try:
        print("🚀 Iniciando Pipeline Port-OUT...")
        ...
        print("🔹 Paso 4: Paso 4: Procesando datos nuevos (Fase pesada)...")
        ...
        conn.commit()
        print("✨ ¡Actualización de tabla Port-OUT completado con éxito!")

    except Exception as e:
        print(f"❌ Error en el proceso OUT: {e}")
    finally:
        cursor.close()
        conn.close()
